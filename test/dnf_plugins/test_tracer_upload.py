...
from dnf_plugins import tracer_upload
...
class TestTracerPlugin(PluginTestCase):
    ...
    @patch('dnf_plugins.tracer_upload.TracerUpload.read_config')
    @patch('dnf_plugins.tracer_upload.upload_tracer_profile')
...
class TestTracerUploadCommand(TestCase):
    def setUp(self):
        self.cli = Mock()
